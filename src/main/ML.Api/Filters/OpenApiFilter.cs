        ...
        foreach (var path in document.Paths)
        {
            foreach (var operation in path.Value.Operations)
            {
                ...
                foreach (var parameter in operation.Value.Parameters)
                {
                    ...
                    {
                        if (parameter.Name.Equals("License", StringComparison.OrdinalIgnoreCase))
                        {
                            ApplyEnumToParameter(parameter, typeof(OpenLicenseEnum));
                        }
                        else if (parameter.Name.Equals("LicenseType", StringComparison.OrdinalIgnoreCase))
                        {
                            ApplyEnumToParameter(parameter, typeof(OpenLicenseTypeEnum));
                        }
                        else if (parameter.Name.Equals("Category", StringComparison.OrdinalIgnoreCase))
                        {
