...
class ChangePasswordTests
{
    ...
    public async Task ChangePassword_PasswordMismatch_FailsPasswordChange()
    {
        _mockIdentityService.Setup(x => x.ChangePasswordAsync(validCommand.NewPassword!))
            .ReturnsAsync((Common.Models.Result.Failure(ResultMessage.ChangePasswordFailed, ["Passwords do not match"]), string.Empty));
    ...
    public async Task ChangePassword_InActiveUser_FailsAccountActivation()
    {
        ...
        _mockIdentityService.Setup(x => x.ChangePasswordAsync(validCommand.NewPassword!))
            .ReturnsAsync((Common.Models.Result.Failure(ResultMessage.ActivateAccountFailed, ["Account is not active"]), string.Empty));
        ChangePasswordCommandHandler handler = new(_mockIdentityService.Object, _mockPublisher.Object);
        var result = await handler.Handle(validCommand, CancellationToken.None);
        Assert.Multiple(() =>
        {
    ...
    {
        _mockIdentityService.Setup(x => x.ChangePasswordAsync(validCommand.NewPassword!))
            .ReturnsAsync((Common.Models.Result.Success(ResultMessage.ChangePasswordSuccess), string.Empty));
        ChangePasswordCommandHandler handler = new(_mockIdentityService.Object, _mockPublisher.Object);
        var result = await handler.Handle(validCommand, CancellationToken.None);
