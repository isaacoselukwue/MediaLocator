namespace ML.Application.FunctionalTests.Accounts.Commands;
[TestFixture]
internal class ActivateAccountTests
{
    ...
    public async Task ActivateAccount_NonExistentUserId_FailsAccountActivation()
    {
        ...
        _mockIdentityService.Setup(x => x.ActivateAccountAsync(validCommand.UserId))
            .ReturnsAsync((Common.Models.Result.Failure(ResultMessage.ActivateAccountFailed, ["Invalid user"]), string.Empty));
    ...
    public async Task ActivateAccount_AlreadyActiveUser_ReturnsUserActive()
    {
        ...
        _mockIdentityService.Setup(x => x.ActivateAccountAsync(validCommand.UserId))
            .ReturnsAsync((Common.Models.Result.Failure(ResultMessage.ActivateAccountFailed, ["Account is already active"]), string.Empty));
        ActivateAccountCommandHandler handler = new(_mockIdentityService.Object, _mockPublisher.Object);
        var result = await handler.Handle(validCommand, CancellationToken.None);
        Assert.Multiple(() =>
        {
    ...
    public async Task ActivateAccount_AccountActivationFails_ReturnsFailedAccount()
    {
        ...
        _mockIdentityService.Setup(x => x.ActivateAccountAsync(validCommand.UserId))
            .ReturnsAsync((Common.Models.Result.Failure(ResultMessage.ActivateAccountFailed, ["Account update error"]), string.Empty));
        ActivateAccountCommandHandler handler = new(_mockIdentityService.Object, _mockPublisher.Object);
        var result = await handler.Handle(validCommand, CancellationToken.None);
        Assert.Multiple(() =>
        {
    ...
    public async Task ActivateAccount_ValidUserId_PublishesUserActivatedEvent()
    {
        ...
        _mockIdentityService.Setup(x => x.ActivateAccountAsync(validCommand.UserId))
            .ReturnsAsync((Common.Models.Result.Success(ResultMessage.ActivateAccountSuccess), users.Email));
        ActivateAccountCommandHandler handler = new(_mockIdentityService.Object, _mockPublisher.Object);
        var result = await handler.Handle(validCommand, CancellationToken.None);
